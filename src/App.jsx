...
export default function App() {
  ...
  const tips = [
    ...
    "Your GPU isn't screaming; it's just singing the song of its people. That VRAM is a lot of room for mistakes.",
    "If the background turns Red, the bot isn't angry—it's just 'strategically failing' into a corner.",
    "The 'Learning...' shimmer is a placebo for your patience. The bot is actually just wondering if it can play CS:GO instead.",
    "Garbage Collection: The only time taking out the trash actually hurts your FPS.",
    "Neural training is 10% math, 90% staring at a Cyan glow hoping for a 2% win rate increase.",
    ...
    "If the UI is janky, blame the Main Thread. It's probably stuck doing manual labor instead of letting the GPU shine.",
    "The 'Neural Bot' is actually just a glorified random number generator with delusions of grandeur. But hey, it's learning!",
    "7 out of 10 AI researchers agree: This is a perfectly valid way to train a neural network in the browser. The other 3 are just haters.",
    "7 quadrillion epochs go!",
    "Just one more layer bro trust me bro it's gonna be so smart bro trust me bro.",
    "Piwka?",
    "Go play Nazi Zombies: Portable, it's free!",
    "If you want to support the project, consider sharing it with your friends or buying me a coffee. Every bit helps and keeps the project alive! https://buymeacoffee.com/doromiert",
    "NOT written in rust.",
    "The AI isn't actually sentient, but it does have feelings. Be nice to it, or it might just refuse to learn.",
    "If you see the bot making really bad moves, don't worry. It's just trying to keep your hopes up before it inevitably crushes you with a perfect strategy.",
    "The training process is like watching a toddler learn to walk. It's messy, it's slow, but eventually it gets there. Just be patient and enjoy the show!",
    "The AI cannot speak. In the event of it communicating with you, please disregard anything it says.",
    "If the bot starts to perform really well, it's not because it's smart. It's probably just memorizing the procedural bots' patterns. Don't worry, it won't be long before it starts to generalize and actually learn to play.",
    "Spice the training up with a few machines! Go to the level editor, export a level as JSON and watch your hours of training go to waste as the bot gets stuck on the concept of a duplicator.",
    "Do you want to play connect-4 instead? Well, we kinda have that too, it's called cascade mode.",
    "I didn't set a board size limit in the editor, but that doesn't mean you should test it with a 100x100 board. The neural bot might just give up and go outside to play instead.",
    "Download more RAM for better results.",
    "I'm such a comedian right? I should probably just stick to coding.",
    "50k epochs and still only 2 wins, FML",
    "Try ZenOS, it comes with absolutely no asbestos inside!",
    ":3",
    "If you see a sudden spike in performance, it's not because the bot got smarter. It's probably just because it accidentally discovered a winning strategy and is now exploiting it to boost its stats. Don't worry, it happens to the best of us.",
    "The bot is actually just an indian guy named Rajesh who is really good at tic tac toe and is doing this for the money. SHIT YOU WEREN'T SUPPOSED TO KNOW THAT",
    "I'm too broke for ts...",
    "WW3? Only if it happens in TTT2 please.",
    "Monero XMR FTW",
    "End the FED",
    "You think you know how to play this game? You're wrong.",
    "Maybe, in the future, scientists will find a way to make the AI not completely and utterly idiotic.",
    "I always wanted to be a developer, but I was too busy being a procrastinator.",
    "I'm writing these funny texts instead of coding because I'm too lazy to actually write code.",
    "There's no tip, keep going.",
    "There's a non-zero chance that this game is actually a simulation of a simulation of a simulation of a simulation of a simulation of a simulation of a simulation of a simulation of a simulation of a simulation of a simulation.",
    "Like AI and video games? Go play Will You Snail? on steam.",
    "This statement is false.",
    "Okay! Here's the seahorse emoji: 🦀 wait that's not the seahorse emoji, the real seahorse emoji is 🦐 wait that's not it either, the real seahorse emoji is...",
    "If you can read this, congratulations! You have a very high attention span. You should probably go outside and enjoy the sun though, it's good for you.",
    "Shlawg what are you doing? Stop looking at these tips and go play the game already!",
    "The bot is actually powered by a hamster running on a wheel in a basement somewhere. The wheel is connected to a generator that powers the computer running the bot. The hamster is fed a steady diet of sunflower seeds and occasionally gets to watch cat videos as a reward for good performance.",
    "The bot is actually kept at a very comfortable 30° at all times, no, not celcius, it's actually kelvin.",
  ];
  ...
  const initializeModel = async () => {
    ...
    // INPUT: Always 81 (The 9x9 local context around a move)
    model.add(
      tf.layers.dense({
        units: 128,
        inputShape: [81],
    ...
    // OUTPUT: 1 (A single Q-value score for the center tile of that 9x9 patch)
    model.add(tf.layers.dense({ units: 1, activation: "linear" }));

    model.compile({
      optimizer: tf.train.adam(0.00025), // Smoother learning rate
      loss: "meanAbsoluteError", // Prevents the 68-billion-point explosions
    });

    modelRef.current = model;
  };
  const getLocal9x9Context = (board, targetX, targetY, player, rows, cols) => {
    const context = new Float32Array(81);
    let i = 0;

    for (let y = targetY - 4; y <= targetY + 4; y++) {
      for (let x = targetX - 4; x <= targetX + 4; x++) {
        if (y < 0 || y >= rows || x < 0 || x >= cols) {
          context[i] = -1.0;
        } else {
          const cell = board[y][x];
          if (cell.piece === player) context[i] = 1.0;
          else if (cell.piece) context[i] = -0.5;
          else if (cell.type === "void" || cell.dead) context[i] = -1.0;
          else context[i] = 0.0;
        }
        i++;
      }
  ...
  const trainOnMemoryBatch = async (batch) => {
  ...
  const executePureTurn = (
    ...
    // 1. Deep clone to prevent history corruption
    let b = inputBoard.map((row) => row.map((cell) => ({ ...cell })));
    let tempScores = { ...currentScores };

    // 2. EXPLICITLY PLACE THE PIECE
    b[startY][startX].piece = player;

    let pointsScoredThisTurn = 0;
    let earnedExtraTurns = 0;
    let newLines = [];

    // 3. YOUR ORIGINAL SAFE RADIAL MATH
    const axes = [
      [
        { dx: 1, dy: 0 },
        { dx: -1, dy: 0 },
    ...
    axes.forEach((axis) => {
      let lineLength = 1;
      let seq = [{ x: startX, y: startY, cell: b[startY][startX] }];
      ...
      axis.forEach((dir) => {
        ...
        while (true) {
          ...
          let currentCell = b[cy][cx];
          let nextCell = b[nextY][nextX];

          // Strict Wall Collisions
          let blocked = false;
          ...
          if (dir.dx === 0 && dir.dy === -1 && nextCell.walls?.b)
            blocked = true;
          if (dir.dx === 1 && dir.dy === 1 && b[cy][nextX]?.walls?.bl)
            blocked = true;
          if (dir.dx === -1 && dir.dy === -1 && b[nextY][cx]?.walls?.bl)
            blocked = true;
          if (dir.dx === -1 && dir.dy === 1 && b[cy][nextX]?.walls?.br)
            blocked = true;
          if (dir.dx === 1 && dir.dy === -1 && b[nextY][cx]?.walls?.br)
            blocked = true;
    ...
    // 5. UPDATE THE SCORE TRACKER
    if (pointsScoredThisTurn > 1) earnedExtraTurns += pointsScoredThisTurn - 1;
    tempScores[player] += pointsScoredThisTurn;

    // 6. CHECK FOR BOARD FULL
    let foundValidMove = false;
    for (let y = 0; y < rows; y++) {
      for (let x = 0; x < cols; x++) {
        const c = b[y][x];
    ...
    return {
      board: b,
      scores: tempScores, // This now safely contains the new point!
      matchOver: !foundValidMove,
      extraTurns: earnedExtraTurns,
      maxComboThisTurn: earnedExtraTurns,
      newLines,
    };
  };
  ...
  const resolveGameStatus = (game, validMovesCount) => {
    ...
    let requiresFullBoard = goals.some((g) => g.type === "standard");
    let forcedEnd = goals.some(
      (g) =>
        g.type === "max_moves" &&
        g.mode === "end" &&
        game.moves >= Number(g.target),
    );
    const isEndState = validMovesCount === 0 || forcedEnd;

    let allMet = true;
    let anyFailed = false;

    goals.forEach((g) => {
      const target = Number(g.target); // [ CRITICAL FIX ]: Ensure target is a real number
      ...
      if (g.type === "standard") {
        ...
        if (isEndState && game.scores.X !== target) anyFailed = true;
      } else if (g.type === "min_score") {
        if (game.scores.X < target) allMet = false;
        if (isEndState && game.scores.X < target) anyFailed = true;
      } else if (g.type === "max_score") {
        if (game.scores.X > target) anyFailed = true;
      } else if (g.type === "fill_targets") {
        if (game.targetsTotal === 0 || game.targetsFilled < game.targetsTotal)
          allMet = false;
        if (
          isEndState &&
          (game.targetsTotal === 0 || game.targetsFilled < game.targetsTotal)
        )
          anyFailed = true;
      } else if (g.type === "min_combo") {
        if (game.maxComboAchieved < target) allMet = false;
        if (isEndState && game.maxComboAchieved < target) anyFailed = true;
      } else if (g.type === "max_moves") {
        if (game.moves > target && (!g.mode || g.mode === "fail"))
          anyFailed = true;
      }
    });
  ...
  useEffect(() => {
    campaignStateRef.current = {
      isActive: isCampaignMode,
      levels: campaignLevels,
      currentIndex: campaignIndex,
      wins: campaignWins,
    };
  }, [isCampaignMode, campaignLevels, campaignIndex, campaignWins]);
  ...
  useEffect(() => {
    ...
    const sampleBatch = (buffer, batchSize) => {
      ...
      if (currentSize <= batchSize) return [...buffer];
      ...
      try {
        ...
        for (let i = 0; i < parallelCount; i++) {
          ...
          if (game.status !== "active") {
            ...
            statsRef.current.history.push(resultValue);

            if (statsRef.current.history.length > 20) {
              statsRef.current.history.shift();
            }

            const recentWins = statsRef.current.history.reduce(
              (a, b) => a + b,
              0,
            );
            statsRef.current.recentWinRate =
              (recentWins + 1) / (statsRef.current.history.length + 1);
            ...
            // Trigger AI Training in the background
            if (
            ...
            continue; // Skip the rest of the loop, this game just restarted
          }

          // IF THE GAME IS ACTIVE: Generate valid moves
        ...
        if (totalNeuralMoves > 0) {
              ...
              for (let y = m.y - 4; y <= m.y + 4; y++) {
                for (let x = m.x - 4; x <= m.x + 4; x++) {
                  if (y < 0 || y >= rows || x < 0 || x >= cols) {
                    ...
                    else if (isInvalidTarget) globalContext[ptr++] = -1.0;
          ...
          activeNeuralTasks.forEach((task) => {
            ...
            const BC_LOYALTY_CHANCE = 0.8; // 80% follow expert, 20% use own brain

            if (
              bcConfigRef.current.enabled &&
              task.game.moves !== 0 &&
              Math.random() < BC_LOYALTY_CHANCE
            ) {
            ...
            // ==========================================
            const validMovesNow = getValidMoves(task.game, rows, cols);
            const newStatus = resolveGameStatus(
              task.game,
              validMovesNow.length,
            );
            task.game.status = newStatus;
            ...
            // Handle turn rotation if the game is still active
  ...
  // --- BATCHED ASYNC HEATMAP (ZERO UI LAG) ---
  const getBoardHeatmapAsync = async (board) => {
    const validCells = [];
    for (let y = 0; y < rows; y++) {
      for (let x = 0; x < cols; x++) {
        const cell = board[y][x];
        if (!cell.piece && !cell.dead && cell.type !== "void") {
          validCells.push({ x, y, idx: y * cols + x });
        }
      }
    }

    const numCells = validCells.length;
    if (numCells === 0) return new Float32Array(cols * rows).fill(0);

    // 1. ONE ALLOCATION (Inlining the loops for raw speed)
    const batchContext = new Float32Array(numCells * 81);
    let ptr = 0;

    for (let i = 0; i < numCells; i++) {
      const { x: targetX, y: targetY } = validCells[i];

      for (let cy = targetY - 4; cy <= targetY + 4; cy++) {
        for (let cx = targetX - 4; cx <= targetX + 4; cx++) {
          if (cy < 0 || cy >= rows || cx < 0 || cx >= cols) {
            batchContext[ptr++] = -1.0;
          } else {
            const c = board[cy][cx];
            if (c.piece === "X") batchContext[ptr++] = 1.0;
            else if (c.piece) batchContext[ptr++] = -0.5;
            else if (c.type === "void" || c.dead) batchContext[ptr++] = -1.0;
            else batchContext[ptr++] = 0.0;
          }
        }
      }
    }

    // 2. ASYNC GPU PREDICTION (No dataSync!)
    const inputTensor = tf.tensor2d(batchContext, [numCells, 81]);
    const predictions = modelRef.current.predict(inputTensor);

    // Yield the thread so CSS animations and UI interactions stay smooth
    ...
    // 3. NORMALIZE SCORES
    const heatmap = new Float32Array(cols * rows).fill(0);
    let minScore = Infinity;
    let maxScore = -Infinity;

    for (let i = 0; i < numCells; i++) {
      const score = scores[i];
      heatmap[validCells[i].idx] = score;
    ...
    if (maxScore > minScore) {
      for (let i = 0; i < numCells; i++) {
        const idx = validCells[i].idx;
        heatmap[idx] = (heatmap[idx] - minScore) / (maxScore - minScore);
      }
    }
                  ...
                  onChange={(e) => {
                    ...
                    reader.onload = (event) => {
                      try {
                        ...
                        // 1. YOUR EXISTING LOGIC: Save it to the app's library
                        const updatedCampaigns = [
                          ...savedCampaigns,
                          { id: generateId(), name, levels: campaignData },
                        ];
                        ...
                        // 2. THE AI HOOKUP: Load Level 1 into the Neural Environment
                        setCampaignLevels(campaignData); // Save the array to state so the AI can progress
                        setCampaignIndex(0);
                        setIsCampaignMode(true);
                        setCampaignWins(0);

                        const firstLevel = campaignData[0];
                        setGameMode(firstLevel.gameMode || "standard");
                        setCols(firstLevel.cols || firstLevel.gridSize || 9);
                        setRows(firstLevel.rows || firstLevel.gridSize || 9);

                        // Load the board state of Level 1
                        if (firstLevel.board) {
                          setBoard(
                            firstLevel.board.map((row) =>
                              row.map((cell) => ({ ...cell })),
                            ),
                          );
                        } else {
                          setBoard(
                            createEmptyBoard(firstLevel.cols, firstLevel.rows),
                          );
                        }
                        ...
                        setAiBehavior(firstLevel.aiBehavior || "standard");

                        alert(
                          `Training Campaign [${name}] loaded!\nLevel 1 queued for the Neural Network.`,
                        );
                      } catch (err) {
                        alert("Invalid campaign JSON file.");
                        console.error(err);
  ...
  if (appMode === "neural_training") {
    return (
      ...
      >
        ...
        <div className="flex justify-between items-center mb-4">
          <div className="flex-1">
            ...
            <p className="font-mono text-xs text-slate-400 mb-2">
              ...
              {isCampaignMode && ` | CAMPAIGN LEVEL: ${campaignIndex + 1}`}
            </p>

            {/* --- CAMPAIGN PROGRESS BAR --- */}
            {isCampaignMode && (
              <div className="w-full max-w-md mb-3">
                <div className="flex justify-between text-[9px] text-fuchsia-400 font-bold tracking-widest uppercase mb-1">
                  <span>Level Mastery Tracker</span>
                  <span>{campaignWins} / 10 WINS</span>
                </div>
                <div className="h-1.5 w-full bg-slate-900 rounded-full overflow-hidden border border-fuchsia-900/50">
                  <div
                    className="h-full bg-fuchsia-500 shadow-[0_0_10px_rgba(217,70,239,0.8)] transition-all duration-300"
                    style={{ width: `${(campaignWins / 10) * 100}%` }}
                  />
                </div>
              </div>
            )}
          ...
          <div className="flex gap-4 h-full">
            ...
            <div className="flex  gap-2">
              ...
              <div className="flex gap-2">
                <button
                  className="px-3 py-1 bg-rose-600/20 text-rose-400 border border-rose-500/50 hover:bg-rose-600/40 rounded font-bold text-[10px] transition-colors flex items-center gap-1"
                  onClick={async () => {
                ...
                <button
                  className="px-3 py-1 bg-emerald-600/20 text-emerald-400 border border-emerald-500/50 hover:bg-emerald-600/40 rounded font-bold text-[10px] transition-colors flex items-center gap-1"
                  onClick={async () => {
        ...
        {/* MATRIX GRID - THE PERFORMANCE FIX */}
        {minimapToggle && (
          <div
            style={{
...
@keyframes explode {
              ...
              >
                <div className="mb-4 space-y-2">
                  ...
                  @keyframes smooth-wave {
                    0%, 30% { transform: scale(1) translateZ(0); opacity: 0.5; }
                    50% { transform: scale(1.1) translateZ(0); opacity: 1; }
                    70%, 100% { transform: scale(1) translateZ(0); opacity: 0.5; }
                  }
  ...
  function getProceduralMove(
    ...
    if (
      !memory.lastBoardPieces ||
      memory.lastBoardPieces.length !== rows ||
      memory.lastBoardPieces[0].length !== cols
    ) {
      // If the board size changed (or it's the first move), completely reset the memory!
      memory.lastBoardPieces = Array.from({ length: rows }, () =>
        new Array(cols).fill(null),
      );

      // (Optional) Reset any other array-based memory your bot uses here
      if (memory.threatMap) memory.threatMap = null;
    }
    ...
    const checkLinePotential = (x, y, pieceToCheck, b) => {
      ...
      // Pre-allocate a fixed array to avoid GC spam
      // 0 = Empty, 1 = Player/Neutral, 2 = Blocked/Enemy
      const segment = new Uint8Array(9);
      ...
      for (let d = 0; d < 4; d++) {
        ...
        // 1. Extract the 9-cell segment into the fixed typed array
        for (let i = -4; i <= 4; i++) {
          let cx = x + i * dx;
          let cy = y + i * dy;
          let idx = i + 4; // Map -4..4 to 0..8

          if (cx >= 0 && cx < cols && cy >= 0 && cy < rows) {
            let cell = b[cy][cx];
            if (cell.type === "void" || cell.dead || cell.type === "trash") {
              segment[idx] = 2; // Blocked
            } else if (cell.piece === pieceToCheck || cell.type === "neutral") {
              segment[idx] = 1; // Player
            } else if (!cell.piece) {
              segment[idx] = 0; // Empty
            } else {
              segment[idx] = 2; // Enemy (Block)
            }
          } else {
            segment[idx] = 2; // Out of bounds
          }
        }
        ...
        // 2. Fast Sliding Window (NO SLICE, NO FILTER)
        for (let size = 5; size >= 1; size--) {
          ...
          for (let start = minStart; start <= maxStart; start++) {
            ...
            // Manual loop replaces .filter() and .slice()
            for (let w = 0; w < size; w++) {
              let val = segment[start + w];
              if (val === 2) {
                blocked = true;
                break;
              } else if (val === 1) {
            ...
            let leftOpen = start > 0 && segment[start - 1] === 0;
            let rightOpen = start + size < 9 && segment[start + size] === 0;
      ...
      return {
        ...
        extraTurns: totalET,
        potentialLines: [], // Stripped out to save memory! Not needed for neural training.
      };
    };
